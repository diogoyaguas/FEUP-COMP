/* Generated By:JavaCC: Do not edit this line. ProgramTreeConstants.java Version 6.0_1 */
package src.AST;

public interface ProgramTreeConstants {
    public int JJTSKIP_TO = 0;
    public int JJTPROGRAM = 1;
    public int JJTCLASS = 2;
    public int JJTEXTENDS = 3;
    public int JJTVAR = 4;
    public int JJTMAIN = 5;
    public int JJTMETHOD = 6;
    public int JJTVOID = 7;
    public int JJTARGUMENT = 8;
    public int JJTRETURN = 9;
    public int JJTIF = 10;
    public int JJTELSE = 11;
    public int JJTWHILE = 12;
    public int JJTASSIGN = 13;
    public int JJTIDENTIFIER = 14;
    public int JJTARRAYASSIGN = 15;
    public int JJTADD = 16;
    public int JJTSUB = 17;
    public int JJTMUL = 18;
    public int JJTDIV = 19;
    public int JJTAND = 20;
    public int JJTLESS_THAN = 21;
    public int JJTEXCLAMATION_MARK = 22;
    public int JJTPERIOD = 23;
    public int JJTNEW = 24;
    public int JJTTERM = 25;
    public int JJTPERIODAUX = 26;
    public int JJTNEWAUX = 27;

    public String[] jjtNodeName = { "skip_to", "Program", "Class", "Extends", "Var", "Main", "Method", "void",
            "Argument", "RETURN", "IF", "ELSE", "WHILE", "ASSIGN", "Identifier", "ArrayAssign", "ADD", "SUB", "MUL",
            "DIV", "AND", "LESS_THAN", "EXCLAMATION_MARK", "PERIOD", "NEW", "Term", "PeriodAux", "NewAux", };
}
/*
 * JavaCC - OriginalChecksum=c74f581c9cc946ff0a559ecaae097420 (do not edit this
 * line)
 */
